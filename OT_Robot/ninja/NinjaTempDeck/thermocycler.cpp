...
// control
void Thermocycler::SetProgram(Cycle* pProgram, Cycle* pDisplayCycle, const char* szProgName) {
  Stop();
...
boolean Thermocycler::Loop() {
  ...
  CheckHardware(&wellTemp);
  PCR_DEBUG("W=");
  PCR_DEBUG_LINE(wellTemp);

  iPlateThermistor.setTemp(wellTemp);

  double estimatedAirTemp = wellTemp;
  // Estimated delta to next 1 sec
  double diff = ((wellTemp - iEstimatedSampleTemp)/THETA_WELL) / CAPACITY_TUBE;
...
void Thermocycler::CheckHardware(float *wellTemp) {
...
#else
  analogWrite(PIN_WELL_PWM, MIN_PELTIER_VALUE/*absOutput*/);
...
void Thermocycler::ProcessCommand(SCommand& command) {
  if (command.command == SCommand::EStart) {
    ...
    GetThermocycler().SetProgram(pProgram, pDisplayCycle, command.name);
