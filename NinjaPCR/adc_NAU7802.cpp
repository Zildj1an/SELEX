...
uint8_t initADC () {
  ...
  */
  //setRegisterBit(0x01, 2);
  //setRegisterBit(0x01,1);
  //setRegisterBit(0x01,0);
  
/*  CALIBRATE
  clearRegisterBit(NAU7802_REG_ADDR_CTRL2, 0);
  clearRegisterBit(NAU7802_REG_ADDR_CTRL2, 1);
  clearRegisterBit(NAU7802_REG_ADDR_CTRL2, 2);

  PCR_ADC_DEBUG_LINE("ABOUT TO CALIBRATE");

  if (waitForFlag(NAU7802_REG_ADDR_CTRL2, 2, false, 1000)) {
    PCR_ADC_DEBUG_LINE("calib SUCCESS");
    bool error = wellADCReadRegValue(NAU7802_REG_ADDR_CTRL2) & 0b00000100;
    if(error) {
      PCR_ADC_DEBUG_LINE("ERROR IN CALIBRATION");
    } else PCR_ADC_DEBUG_LINE("NO ERROR IN CALIBRATION");
  } else {
    PCR_ADC_DEBUG_LINE("calib FAIL");
    delay(500000);
  }*/

  //wellADCWriteRegValue(0x03, 0x00);
